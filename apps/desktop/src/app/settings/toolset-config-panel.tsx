...
function PostSetupRunner({ toolset, postSetupKey, onComplete }: PostSetupRunnerProps) {
  ...
  const run = useCallback(async () => {
    ...
    try {
      ...
      if (activeRef.current) {
        ...
        notify(
          ok
            ? { kind: 'success', title: 'Setup complete', message: `${postSetupKey} installed.` }
            : { kind: 'error', title: 'Setup finished with errors', message: `Check the ${postSetupKey} log.` }
        )
        onComplete?.()
      }
    } catch (err) {
      if (activeRef.current) {
        notifyError(err, `Failed to run ${postSetupKey} setup`)
      }
    } finally {
      if (activeRef.current) {
        setRunning(false)
  ...
  }, [toolset, postSetupKey, onComplete])
      ...
      <div className="flex flex-wrap items-center justify-between gap-2">
        <div className="min-w-0">
          <p className="text-[0.72rem] text-muted-foreground">
            This backend needs a one-time install (<code className="font-mono">{postSetupKey}</code>). Runs on this
            machine — may take a few minutes.
          </p>
        </div>
